...
### Готовый бинарник
Скачайте последний релиз со страницы [Releases](https://github.com/Hr0mE/vk-poll-tracker/releases) под вашу платформу и запустите — всё уже внутри, Python устанавливать не нужно.
...
### Как узнать Peer ID беседы
Peer ID групповой беседы = `2000000000 + локальный номер беседы`.

Локальный номер можно найти в URL при открытии беседы через браузер:
`https://vk.com/im?sel=c<число>` — вот это число и нужно прибавить к двум миллиардам.

Либо через [VK API Explorer](https://vk.com/dev/methods) вызовите `messages.getConversations` и найдите нужную беседу в ответе.
...
### Результат
Файл `report.xlsx` с двумя листами:
...
made by [Fes](https://github.com/Hr0mE) with ♥
