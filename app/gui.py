...
from PyQt6.QtCore import (
    QByteArray, QDate, QRectF, QThread, Qt, QTimer, QUrl, pyqtSignal,
)
from PyQt6.QtGui import (
    QColor, QConicalGradient, QDesktopServices, QIcon, QPainter, QPen,
...
QPushButton#gh_icon:hover {{ background-color: {SURFACE}; }}
"""

_GITHUB_SVG = b"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
<path fill="#a6adc8" d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 \
...
2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55\
.38A8.013 8.013 0 0 0 16 8c0-4.42-3.58-8-8-8z"/>
</svg>"""


def _make_github_icon(size: int = 16) -> QIcon | None:
    try:
        from PyQt6.QtSvg import QSvgRenderer
        renderer = QSvgRenderer(QByteArray(_GITHUB_SVG))
...
class SettingsDialog(QDialog):
    def __init__(self, parent: QWidget | None = None, first_run: bool = False) -> None:
        ...
        if first_run:
            token_link = QLabel(
                f'Получить токен: <a href="https://vkhost.github.io" style="color: {BLUE};">vkhost.github.io</a>'
            )
            token_link.setOpenExternalLinks(True)
            token_link.setObjectName("subtitle")
        ...
        gh_btn = QPushButton()
        gh_btn.setObjectName("gh_icon")
        gh_btn.setFixedSize(20, 20)
        gh_btn.setToolTip("GitHub: Hr0mE")
        gh_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        _icon = _make_github_icon(14)
        if _icon:
            gh_btn.setIcon(_icon)
        else:
            gh_btn.setText("GH")
        gh_btn.clicked.connect(lambda: QDesktopServices.openUrl(QUrl("https://github.com/Hr0mE")))
        footer_row.addWidget(gh_btn)

        credit_lbl = QLabel(
            f'made by <a href="https://vk.com/festfull" style="color: {ACCENT};">Fes</a> with ♥'
        )
        credit_lbl.setOpenExternalLinks(True)
        credit_lbl.setObjectName("subtitle")
        footer_row.addWidget(credit_lbl)
            ...
            async with VKClient(self.token, settings.vk_api_version, limiter) as client:
                ...
                for i, poll in enumerate(polls):
                    ...
                    try:
                        ...
                        self.log_line.emit(
                            f"  ↳ Нет доступа к опросу от {poll.date.strftime('%d.%m.%Y')} — всем проставлено н/д", "warn"
                        )
                        records = [
                            Record(user_id=uid, poll_date=poll.date, status=VoteStatus.NA)
                            for uid in members
                        ]
                    except Exception as e:
                        self.log_line.emit(
                            f"  ↳ Не удалось получить данные ({e}) — всем проставлено н/д", "warn"
                        )
                        records = [
                            Record(user_id=uid, poll_date=poll.date, status=VoteStatus.NA)
                            for uid in members
                ...
                if not output_dir.exists():
                    ...
                    self.log_line.emit(f"Создана папка: {output_dir}", "warn")
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        _app_dir = Path(sys.executable).parent if getattr(sys, "frozen", False) else Path(sys.argv[0]).parent
        default_output = str(_app_dir / "report.xlsx")
